...
import {
  ...
  ChevronRight, Home, Menu, X, AlertCircle,
  Clock, Activity,
    ...
    <section className="mb-8">
      ...
      {filter !== "Announcement" && error && (
        <div className={`mb-3 px-4 py-3 rounded-xl border flex items-start gap-2 text-xs ${l ? "bg-amber-50 border-amber-200 text-amber-700" : "bg-amber-500/10 border-amber-500/20 text-amber-400"}`}>
          ...
          <div>
            <p className="font-bold">Corporate actions temporarily unavailable ({error})</p>
            <p className="mt-0.5 opacity-80">NSE archive fetch failed. You can view data directly on NSE/BSE websites:</p>
          ...
          {loading ? (
            ...
            <div className={`p-10 text-center ${tx.t2(l)}`}>
              ...
              <p className="text-xs mt-1">
                {error ? "Archive fetch failed — retry karo ya NSE/BSE directly dekho" : "No events match the selected filter"}
              </p>
