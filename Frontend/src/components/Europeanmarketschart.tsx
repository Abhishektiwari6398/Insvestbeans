// EuropeanMarketsChart.tsx
// Dedicated chart for European Markets
// Symbols: ^FTSE (FTSE 100), ^GDAXI (DAX), ^FCHI (CAC 40)
// IST Timings:
//   FTSE 100: 12:30 PM – 9:00 PM IST (Summer) | 1:30 PM – 10:00 PM IST (Winter)
//   DAX:      12:30 PM – 9:00 PM IST (Summer) | 1:30 PM – 10:00 PM IST (Winter)
//   CAC 40:   12:30 PM – 9:00 PM IST (Summer) | 1:30 PM – 10:00 PM IST (Winter)

import CleanChart from "@/components/CleanChart";
import { IndexQuote } from "@/services/globalMarkets/types";
import {
  TrendingUp, TrendingDown, LineChart, Clock, AlertTriangle,
...
// ── IST Timings per exchange (from user-provided reference) ──────
// FTSE, DAX, CAC all open roughly same time in IST
const EU_IST_TIMINGS = {
  summer: { open: "12:30 PM IST", close: "9:00 PM IST",  label: "Summer/BST/CEST" },
  winter: { open: "1:30 PM IST",  close: "10:00 PM IST", label: "Winter/GMT/CET"  },
};

// ── Per-symbol exchange label ─────────────────────────────────────
...
// ── Check if London market is open (used as proxy for all EU) ────
function isEUMarketOpen(): "open" | "pre" | "closed" {
  const now = new Date();
  ...
  const mins = h * 60 + mn;
  if (mins >= 480 && mins < 990) return "open";  // 08:00–16:30 GMT/BST
  if (mins >= 450 && mins < 480) return "pre";
  return "closed";
}

// ── Is London on BST (summer)? ────────────────────────────────────
function isLondonSummer(): boolean {
  return getUTCOffset("Europe/London") === 1; // BST = UTC+1
}
...
// ══════════════════════════════════════════════════════════════════
interface EuropeanMarketsChartProps {
  markets:        IndexQuote[];
...
export function EuropeanMarketsChart({ markets, onChart, autoSym, regionSummary }: EuropeanMarketsChartProps) {
  const l = useIL();
  ...
  const [mktStatus, setMktStatus] = useState<"open"|"pre"|"closed">(isEUMarketOpen());
  useEffect(() => {
    const t = setInterval(() => setMktStatus(isEUMarketOpen()), 60_000);
    return () => clearInterval(t);
  }, []);

  const isSummer = isLondonSummer();
  const timing   = isSummer ? EU_IST_TIMINGS.summer : EU_IST_TIMINGS.winter;
  ...
  const fetchCandles = useCallback(async (symbol: string, p: Period) => {
    ...
    try {
      ...
      if (id !== fetchIdRef.current) return;
      console.error(`[EuropeanMarketsChart] fetch failed ${symbol} ${p}:`, e);
      setFetchError(true);
    } finally {
  ...
  if (!markets.length) return (
    <Card className="py-10 text-center">
      <LineChart className={`w-8 h-8 mx-auto mb-2 ${tx.t3(l)}`}/>
      <p className={`text-sm ${tx.t2(l)}`}>No European market data</p>
    </Card>
  );

  const s     = markets[sel];
  const isPos = s.changePercent >= 0;
  const symTz = SYMBOL_TZ[s.symbol] ?? "Europe/London";
  const exLabel = EXCHANGE_LABEL[s.symbol] ?? s.name;
        ...
        <div className={`flex items-center justify-between px-4 py-2 border-b ${tx.header(l)} border-b`}>
          <div className="flex items-center gap-2 flex-wrap">
            ...
            {/* <div className={`flex items-center gap-1 text-[9px] font-semibold ${tx.t3(l)}`}>
              ...
              <span>
                {exLabel} — {timing.open} – {timing.close}
...
export default EuropeanMarketsChart;