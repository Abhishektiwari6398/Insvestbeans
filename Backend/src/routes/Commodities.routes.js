...
// ── AMFI ETF Inflow — 100% Live, Zero Hardcoded ──────────
// ── AMFI ETF Inflow — 100% Live, Zero Hardcoded ──────────
const AMFI_INFLOW_TTL = 24 * 60 * 60 * 1000;
let _inflowCache = null, _inflowTs = 0;

const MON       = ["jan","feb","mar","apr","may","jun","jul","aug","sep","oct","nov","dec"];
const MON_LABEL = ["Jan","Feb","Mar","Apr","May","Jun","Jul","Aug","Sep","Oct","Nov","Dec"];

async function fetchAMFIInflowHistory() {
  if (_inflowCache && Date.now() - _inflowTs < AMFI_INFLOW_TTL) return _inflowCache;
  ...
  for (let offset = 1; offset <= 24; offset++) {
    ...
    const mi    = d.getMonth();
    const label = `${MON_LABEL[mi]} ${yr}`;
    const urlMain = `https://portal.amfiindia.com/spages/am${MON[mi]}${yr}repo.xls`;
    const urlSub  = `https://portal.amfiindia.com/spages/am${MON[mi]}${yr}subclassrepo.xls`;
    ...
    try {
      ...
      console.log(`[AMFI Silver] ${label} sub URL status: ${res.status} | ${urlSub}`);
      if (res.ok) {
        ...
        if (label === "Mar 2026") {
          rows.forEach((row, i) => {
            ...
            if (cells.some(c => c.length > 0)) { // sirf non-empty rows
              console.log(`SUB ROW ${i}:`, cells.slice(0, 6));
            }
          });
        }
        for (const row of rows) {
          const cells = row.map(c => String(c ?? "").replace(/,/g, "").trim());
          // ✅ cells[1] = Scheme Name, cells[4] = Sales, cells[5] = Redemption
          if (cells[1]?.toLowerCase() === "gold etf") {
            const sales  = parseFloat(cells[4]);
            const redeem = parseFloat(cells[5]);
            if (!isNaN(sales) && !isNaN(redeem)) {
              const net = sales - redeem; // Net inflow in crore
              gold.unshift({ month: label, inflow: +(net / 100).toFixed(2) }); // crore → billion
              console.log(`[AMFI] Gold ${label}: ${net.toFixed(0)} cr = ${+(net/100).toFixed(2)}B`);
            }
            break;
          }
        }
      ...
      console.error(`[AMFI Gold] ${label}: ${e.message}`);
    }

    // ── Silver ETF: Sub-Classification Excel ─────────────
   // ── Silver ETF: Sub Classification of Other Scheme Excel ─
// ── Silver ETF: Sub-Classification of Other Scheme Excel ─
try {
  // URL pattern: Aug2025+ = Capital (Feb26, Jan26, Dec25...)
  //              Before Aug2025 = lowercase (jul25, jun25, may25...)
  const MONTH_ABBR_CAP = ["Jan","Feb","Mar","Apr","May","Jun","Jul","Aug","Sep","Oct","Nov","Dec"];
  const MONTH_ABBR_LOW = ["jan","feb","mar","apr","may","jun","jul","aug","sep","oct","nov","dec"];

  // Aug 2025 = index 7, year 2025 — pehle lowercase, baad capital
  const isCapital = (yr > 2025) || (yr === 2025 && mi >= 7);
  const monAbbr   = isCapital ? MONTH_ABBR_CAP[mi] : MONTH_ABBR_LOW[mi];
  const yrShort   = String(yr).slice(2); // 2025 → "25"
  const urlSilver = `https://portal.amfiindia.com/spages/Sub-classification-${monAbbr}${yrShort}.xlsx`;

  const res = await fetch(urlSilver, {
    headers: { "User-Agent": "Mozilla/5.0" },
    signal: AbortSignal.timeout(8000),
  });

  console.log(`[AMFI Silver] ${label}: ${res.status} → ${urlSilver}`);
  ...
  if (res.ok) {
    ...
    for (const row of rows) {
      const cells = row.map(c => String(c ?? "").replace(/,/g, "").trim());
      const lbl   = cells[1]?.toLowerCase() ?? "";
      if (lbl.includes("silver etf") && !lbl.includes("fof") && !lbl.includes("fund of")) {
        const sales  = parseFloat(cells[4]);
        const redeem = parseFloat(cells[5]);
        if (!isNaN(sales) && !isNaN(redeem)) {
          const net = sales - redeem;
          silver.unshift({ month: label, inflow: +(net / 100).toFixed(2) });
          console.log(`[AMFI] Silver ${label}: ${net.toFixed(0)} cr = ${+(net/100).toFixed(2)}B`);
        }
        break;
      }
    }
  ...
  console.error(`[AMFI Silver] ${label}: ${e.message}`);
}
  }

  // console.log(`[AMFI Inflow] Done — Gold: ${gold.length} months, Silver: ${silver.length} months`);
  _inflowCache = { gold, silver };
  _inflowTs    = Date.now();
  return _inflowCache;
}
...
function classifyRegime(macro) {
  ...
  if (vixPrice > 30) {
    regime = {
      label: "War / Risk-Off Premium", color: "#ef4444", emoji: "⚠️",
    ...
    regime = {
      label: "Inflation Expansion", color: "#f97316", emoji: "📈",
    ...
    regime = {
      label: "Growth Boom", color: "#22c55e", emoji: "🚀",
    ...
    regime = {
      label: "Stable / Transition", color: "#3b82f6", emoji: "⚖️",
  ...
  try {
    ...
    const domestic = await Promise.all(
      MCX_COMMODITIES.map(async (cfg) => {
        ...
        let inrPrice, inrPrev, inrHigh, inrLow;
        if (dataSource === "kite-mcx") {
          inrPrice = Math.round(livePrice);
          inrPrev = Math.round(livePrev);
          inrHigh = Math.round(liveHigh);
          inrLow = Math.round(liveLow);
        } else {
          inrPrice = cfg.inrConvert ? cfg.inrConvert(livePrice, USD_INR) : null;
          inrPrev = cfg.inrConvert ? cfg.inrConvert(livePrev, USD_INR) : null;
          inrHigh = cfg.inrConvert ? cfg.inrConvert(liveHigh, USD_INR) : null;
          inrLow = cfg.inrConvert ? cfg.inrConvert(liveLow, USD_INR) : null;
        }
    ...
    const liveInflows = await fetchAMFIInflowHistory().catch(() => ({ gold: [], silver: [] }));
    ...
    const intelligence = {
      ...
      silverInflowHistory: liveInflows.silver,
      goldInflowHistory: liveInflows.gold,
      cotSummary: {
...
router.get("/silver-inflows", async (req, res) => {
  const inflows = await fetchAMFIInflowHistory().catch(() => ({ gold: [], silver: [] }));
  res.json({
    data:   inflows.silver,
    unit:   "Billion INR",
    source: "AMFI India (live)",
  });
});
